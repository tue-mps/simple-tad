...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, ceph_args={'use_ceph_checkpoint': False}, latest=False):
    output_dir = Path(args.output_dir)
    ...
    if ceph_args['use_ceph_checkpoint']:
        ...
        if loss_scaler is not None:
            ...
            if not flag:
                try:
                    load_specific_model(model, model_ema, args, output_dir, model_name='last')
                    flag = True
                except Exception:
                    print('No latest model')
            if not flag:

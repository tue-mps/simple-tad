...
from utils import gather_predictions_nontensor
...
def train_one_epoch(
    ...
    for data_iter_step, (samples, targets, _, extra_info) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            loss /= update_freq
            model.backward(loss)
            # works only with zero stage 0
            grad_norms = utils1.collect_grad_norms(model, num_layers=12, num_heads=6)
            model.step()
...
@torch.no_grad()
def validation_one_epoch(data_loader, model, device, with_ttc=False, smoothed_labels_for_loss=False, ds=False, bf16=False):
...
@torch.no_grad()
def final_test(data_loader, model, device, preds_file, stats_file, plot_dir=None, with_ttc=False, smoothed_labels_for_loss=False, ds=False, bf16=False):
    criterion = torch.nn.CrossEntropyLoss()

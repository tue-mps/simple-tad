...
from other_models.InternVideo2_single_modality.datasets.mixup import Mixup
from other_models.InternVideo2_single_modality.optim_factory import create_optimizer, get_parameter_groups, LayerDecayValueAssigner
from other_models.InternVideo2_single_modality.engines.iv2_engine_for_frame_finetuning import train_one_epoch, validation_one_epoch, final_test
from other_models.InternVideo2_single_modality.iv2_utils import NativeScalerWithGradNormCount as NativeScaler
from other_models.InternVideo2_single_modality.iv2_utils import multiple_samples_collate
from other_models.InternVideo2_single_modality import iv2_utils as utils
from other_models.InternVideo2_single_modality.models import inflate_weight
...
def main(args, ds_init):
    ...
    utils.auto_load_model(
        ...
        ceph_args=ceph_args, latest=False
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if max_auc < test_stats["auroc"]:
                max_auc = test_stats["auroc"]
                if args.output_dir and args.save_ckpt:
                    utils1.save_model_weights_only(
                        args=args, model_without_ddp=model_without_ddp, epoch="bestauroc")
            if max_ap < test_stats["ap"]:
                max_ap = test_stats["ap"]
                if args.output_dir and args.save_ckpt:
                    utils1.save_model_weights_only(
                        args=args, model_without_ddp=model_without_ddp, epoch="bestap")
            if max_acc < test_stats["metr_acc"]:
                max_acc = test_stats["metr_acc"]
                if args.output_dir and args.save_ckpt:
                    utils1.save_model_weights_only(
                        args=args, model_without_ddp=model_without_ddp, epoch="bestacc")
            if max_mcc_auc < test_stats["mcc_auc"]:
                max_mcc_auc = test_stats["mcc_auc"]
                if args.output_dir and args.save_ckpt:
                    utils1.save_model_weights_only(
                        args=args, model_without_ddp=model_without_ddp, epoch="bestmccauc")
            ...
            if log_writer is not None:
                ...
                log_writer.update(val_mcc_auc=test_stats['mcc_auc'], head="val", step=epoch)
                log_writer.update(val_mcc_max=test_stats['mcc_max'], head="val", step=epoch)
                log_writer.update(val_mcc_max_thresh=test_stats['mcc_max_thresh'], head="val", step=epoch)
                log_writer.update(val_mcc_05=test_stats['mcc_05'], head="val", step=epoch)
                #

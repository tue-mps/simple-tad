...
from engine_for_frame_finetuning import train_one_epoch, validation_one_epoch, final_test
from utils import NativeScalerWithGradNormCount as NativeScaler
from utils import multiple_samples_collate
import utils

import other_models.MVD.modeling_finetune
...
def main(args, ds_init):
    ...
    if not args.eval:
        ...
        if (dataset_val is not None) and args.dist_eval:
    ...
    bs_k = 2 if "large" in args.model else 4

    if dataset_val is not None:
        data_loader_val = torch.utils.data.DataLoader(
            dataset_val, sampler=sampler_val,
            batch_size=int(bs_k * args.batch_size),
            num_workers=args.num_workers,
            pin_memory=args.pin_mem,
            drop_last=False
        )
    ...
    if args.finetune:
        ...
        for key in all_keys:
            ...
            if key.startswith('backbone.'):
                ...
                new_dict[key.replace("norm.", "fc_norm.")] = checkpoint_model[key]
    ...
    utils.auto_load_model(
        ...
        optimizer=optimizer, loss_scaler=loss_scaler, model_ema=model_ema, latest=False)
